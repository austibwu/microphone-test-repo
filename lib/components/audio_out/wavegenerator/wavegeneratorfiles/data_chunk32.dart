...
class DataChunk32 implements DataChunk {
  ...
  // stored as signed bytes, -2^32 < x < 2^32 - 1
  ...
  const DataChunk32(this.format, this.note);
  ...
  Stream<int> bytes() async* {
    ...
    byteData.setInt32(0, length, Endian.little);
    for (int i = 0; i < 4; i++) {
    ...
    for (int x = 0; x < totalSamples; x++) {
      ...
      var byteData = ByteData(4);
      byteData.setInt32(0, sampleByte, Endian.little);
      for (int i = 0; i < 4; i++) {
        yield byteData.getInt8(i);
      }
    }
